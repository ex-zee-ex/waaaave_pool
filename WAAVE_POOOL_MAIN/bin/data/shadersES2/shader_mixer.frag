...
void main()
{
	...
	//testing out downscaling stuffs for vidgrabber
	//vec4 cam1_color = texture2D(tex0, texCoordVarying*vec2(2.0,2.0));
	...
	if(toroid_switch==1){
		if(abs(fb0_coord.x)>1.0){fb0_coord.x=abs(1.0-fb0_coord.x);}
		if(abs(fb0_coord.y)>1.0){fb0_coord.y=abs(1.0-fb0_coord.y);}
